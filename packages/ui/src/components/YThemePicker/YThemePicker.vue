    ...
    <YSection v-show="border" title="Border">
      <YRow type="cluster">
        <div>
          <h2>Border</h2>
          <ul>
            <li v-for="(value, name) in Object.entries(border)" :key="name">
              {{ name }}: {{ value }}
            </li>
          </ul>
        </div>
        ...
        <div>
          <p>Font</p>
          <ul>
            <li v-for="(value, name) in Object.entries(font)" :key="name">
              {{ name }}: {{ value }}
            </li>
          </ul>
        </div>
        <div>
          <p>Spacing</p>
          <ul>
            <li v-for="(value, name) in Object.entries(spacing)" :key="name">
              {{ name }}: {{ value }}
            </li>
          </ul>
        </div>
        <div>
          <p>Radius</p>
          <ul>
            <li v-for="(value, name) in Object.entries(radius)  " :key="name">{{ name }}: {{ value }}</li>
          </ul>
        </div>
        <div>
          <p>Breakpoint Pool</p>
          <ul>
            <li v-for="(value, name) in Object.entries(breakpointPool)" :key="name">
              {{ name }}: {{ value }}
            </li>
          </ul>
        </div>
